extends Node
class_name EndlessTowerProgram
...
@onready var endless_program:AnchorProgram = $ENDLESS_PROGRAM

func spawn_anchor_program_instance()->AnchorProgram:
	var instance:AnchorProgram = AnchorProgram.new()
	add_child(instance)
	instance.set_pid(endless_program.get_pid())
	instance.set_json_file(endless_program.get_json_file())
	instance.set_idl(endless_program.get_idl())
	return instance

func get_pid() -> Pubkey:
		return Pubkey.new_from_string(endless_program.get_pid())
...
func init_game(game_attributes:EndlessUtils.GameAttributes, game_auth:Keypair, wallet:Pubkey) -> TransactionData:
	var treasury_account:Pubkey = EndlessPDA.treasury_pda(get_pid())
	var position_account:Pubkey = EndlessPDA.position_pda(wallet, get_pid())
	var instructions:Array[Instruction]
	var init_game_ix:Instruction = endless_program.build_instruction("rank",[
		#SolanaService.wallet.get_pubkey(), #payer
		game_auth, #authority
		treasury_account, #treasury_data
		wallet, #wallet
		position_account, #position_data
		Pubkey.new_from_string("11111111111111111111111111111111") #system program
	],{
		"floor":game_attributes.floor,
		"name":game_attributes.name,
	})
	
	#print("Creating Game Account with ID: %s"%treasury_account.get_public_string())
	print("GameAttributes: %s"%game_attributes.floor)
	print("GameAttributes: %s"%game_attributes.name)
	print("Payer: %s"%SolanaService.wallet.get_kp().to_string())
	print("treasury_account: %s"%treasury_account.to_string())
	print("Wallet: %s"%SolanaService.wallet.get_pubkey().to_string())
	print("position_account: %s"%position_account.to_string())
...
func add_leaderboard(game_address:String,leaderboard_data:SoarUtils.LeaderboardData,game_auth:Keypair) -> TransactionData:
	...
	var instructions:Array[Instruction]
	var add_leaderboard_ix:Instruction = endless_program.build_instruction("addLeaderboard",[
...
func update_leaderboard(game_address:String,leaderboard_address:String,leaderboard_data:SoarUtils.LeaderboardData,game_auth:Keypair) -> TransactionData:
	...
	var instructions:Array[Instruction]
	var update_leaderboard_ix:Instruction = endless_program.build_instruction("updateLeaderboard",[
		game_auth, #game authority
		game_account, #game
...
func initialize_player(username:String, user_nft:Pubkey) -> TransactionData:
	...
	var instructions:Array[Instruction]
	var init_player_ix:Instruction = endless_program.build_instruction("initializePlayer",[
		SolanaService.wallet.get_kp(), #payer
...
func update_player(username:String, user_nft:Pubkey) -> TransactionData:
	...
	var instructions:Array[Instruction]
	var update_player_ix:Instruction = endless_program.build_instruction("updatePlayer",[
		SolanaService.wallet.get_kp(), #user
		player_account, #player PDA
	],{
...
func submit_score_to_leaderboard(game_account:Pubkey,leaderboard_account:Pubkey,game_auth:Keypair,score:int) -> TransactionData:
	...
	if player_scores_list.size()==0:
		var register_player_ix:Instruction = endless_program.build_instruction("registerPlayer",[
		SolanaService.wallet.get_kp(), #payer
		SolanaService.wallet.get_kp(), #user
	...
	var submit_score_ix:Instruction = endless_program.build_instruction("submitScore",[
		SolanaService.wallet.get_kp(), #payer
		game_auth, #authority
		player_account_pda, #player PDA
		game_account, #game account 
