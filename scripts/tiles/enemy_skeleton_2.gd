...
var strong    : int = 10  #ENEMY POWER
#var is_critical: int = 8 #ENEMY CRIT POWER
var health    : int = 40  #ENEMY HEALTH
...
func _attack() -> void:
	...
	# Escolhe uma animação aleatoriamente cada vez que o inimigo atacar
	var chosen_attack = "attack1" if randi() % 2 == 0 else "attack2"
	
	target.apply_damage(strong, true)
	animator.play(chosen_attack)
	await animator.animation_finished
	animator.play ("idle")
	
	#APPLY DAMAGE ON ENEMY
func apply_damage(player_damage: int) -> void:
	# Verificar se o inimigo irá bloquear o ataque
	if _should_block():
		# Executa a animação de block e evita que o dano seja aplicado
		animator.play ("block")
		await animator.animation_finished
		animator.play ("idle")
		return  # O inimigo não leva dano e a função termina aqui
...
func _create_effect() -> void:
	...
	#effect.tween_property(self, "scale", Vector2.ONE / 1, time_effect).set_trans(Tween.TRANS_ELASTIC).set_ease(Tween.EASE_OUT)
...
func _should_block() -> bool:
	# 30% de chance de bloquear o ataque
	return randf() < 0.3
